...
import org.stg.service.generator.account.FSCAccountGeneratorService;

import com.sforce.async.OperationEnum;

@Service
public class BulkFSCAccountService extends BaseIndustryService{

    final static Logger logger = Logger.getLogger(BulkFSCAccountService.class);

    @Autowired
    IndustriesAccountService accountService;
    @Autowired
    FSCAccountGeneratorService fscAccountGeneratorService;
    @Autowired
    ...
    public List<IndividualResult> generateAndPush(int count) throws Exception {
        ...
        if(recordType != null) {
            List<FSCAccount> accounts = fscAccountGeneratorService.generate(count, recordType, nameSpace,null);
            BulkOperation<FSCAccount> acctBulk = new BulkOperation<FSCAccount>(nameSpace);
